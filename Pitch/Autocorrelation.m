...
period1=zeros(1,fn);                             % 基音周期初始化
period1=ACF_corr(yy,fn,voiceseg,vosl,lmax,lmin); % 用自相关函数提取基音周期
T01=pitfilterm1(period1,voiceseg,vosl);           % 平滑处理

period2=zeros(1,fn);                             % 基音周期初始化
period2=ACF_clip(yy,fn,voiceseg,vosl,lmax,lmin); % 用自相关函数提取基音周期
T02=pitfilterm1(period2,voiceseg,vosl);           % 平滑处理

period3=zeros(1,fn);                             % 基音周期初始化
period3=ACF_threelevel(yy,fn,voiceseg,vosl,lmax,lmin); % 用自相关函数提取基音周期
T03=pitfilterm1(period3,voiceseg,vosl);           % 平滑处理

% 作图
figure(1);
subplot 211, plot(time,x,'k');  title('语音信号')
axis([0 max(time) -1 1]); grid;  ylabel('幅值');
subplot 212; plot(frameTime,T01,'k'); hold on;
...
subplot 211, plot(time,x,'k');  title('语音信号')
axis([0 max(time) -1 1]); grid;  ylabel('幅值');
subplot 212; plot(frameTime,T02,'k'); hold on;
xlim([0 max(time)]); title('平滑后的基音周期'); 
grid; xlabel('时间/s'); ylabel('样点数');
for k=1 : vosl
    nx1=voiceseg(k).begin;
...
subplot 211, plot(time,x,'k');  title('语音信号')
axis([0 max(time) -1 1]); grid;  ylabel('幅值');
subplot 212; plot(frameTime,T03,'k'); hold on;
xlim([0 max(time)]); title('平滑后的基音周期'); 
grid; xlabel('时间/s'); ylabel('样点数');
for k=1 : vosl
    nx1=voiceseg(k).begin;
